...
function status(options) {
  ...
  if (pastDueOneShots.length > 0) {
    ...
    for (const job of pastDueOneShots.slice(0, 3)) {
      const scheduled = formatNextRun(job.next_run_at);
      console.log(`  ⚠️  ${job.name}`);
      console.log(`      Scheduled: ${scheduled} (missed)`);
    }
    if (pastDueOneShots.length > 3) {
      console.log(`  ... and ${pastDueOneShots.length - 3} more`);
    }
    console.log('');
  }
  ...
  if (completedOneShots.length > 0) {
    ...
    for (const job of completedOneShots.slice(0, 5)) {
    ...
    if (completedOneShots.length > 5) {
      console.log(`  ... and ${completedOneShots.length - 5} more`);
    }
    console.log('');
