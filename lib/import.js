...
const { importHeartbeats } = require('./import-heartbeat');
...
function importJobs(options) {
  ...
  if (options.heartbeatOnly) {
    return importHeartbeatOnly(options);
  }

  // Handle unified import (heartbeat + cron)
  if (options.heartbeat) {
    return importUnified(options);
  }

  // Default: cron-only import (existing behavior)
  return importCronOnly(options);
}

function importHeartbeatOnly(options) {
  const configPath = resolveConfigPath(options.config);
  const result = importHeartbeats(configPath, options);
...
function importUnified(options) {
  console.log('🫀 CardioClaw Unified Import\n');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

  // First import heartbeats
  console.log('📍 Phase 1: Heartbeat Configs\n');
  importHeartbeatOnly(options);

  // Then import cron jobs
  console.log('\n📍 Phase 2: Cron Jobs\n');
  importCronOnly(options);

  console.log('\n✅ Unified import complete\n');
}

function importCronOnly(options) {
  const configPath = resolveConfigPath(options.config);
  ...
  const newConfig = { ...existingConfig, heartbeats: allHeartbeats };
