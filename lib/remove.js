      ...
      if (!config.heartbeats || !Array.isArray(config.heartbeats)) {
        ...
        if (before === after) {
          ...
          if (options.dryRun) {
            ...
            // Preserve comments by doing a simple regex replace instead of full YAML rewrite
            // This is a bit crude but preserves formatting better
            const newContent = removeHeartbeatFromYaml(content, name);
