...
function archiveCompletedOneShots(configPath) {
  ...
  try {
    ...
    const jsonStr = jsonStartIdx > 0 ? result.substring(jsonStartIdx) : result;
  ...
  for (const heartbeat of config.heartbeats) {
    ...
    if (!heartbeat.schedule || !heartbeat.schedule.startsWith('at ')) {
    ...
    const scheduledTime = new Date(heartbeat.schedule.replace('at ', '').trim());
    ...
    const isDeleted = job.deleteAfterRun === false; // Check if it ran and was set to delete
