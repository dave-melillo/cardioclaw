      ...
      for (const cardio of cardios) {
        ...
        try {
          if (cardio.type === 'cron') {
            ...
            // Heartbeats run at intervals - calculate all occurrences in range
            const intervalMs = parseIntervalToMs(cardio.schedule);
            if (intervalMs <= 0) continue;
            
            // Start from beginning of range and calculate all occurrences
            // Align to interval boundaries from midnight
            const dayStart = new Date(startDate);
            dayStart.setHours(0, 0, 0, 0);
            const dayStartMs = dayStart.getTime();
            
            // Calculate first occurrence on or after startDate
            const offsetFromMidnight = (startDate.getTime() - dayStartMs) % intervalMs;
            let current = startDate.getTime() - offsetFromMidnight;
            if (current < startDate.getTime()) current += intervalMs;
