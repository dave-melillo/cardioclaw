...
function createDashboardApp(options = {}) {
  ...
  // Heartbeat endpoints (new in v0.4.0)
  app.get('/api/heartbeats', (req, res) => {
    try {
...
function startDashboard(options) {
  ...
  app.listen(port, host, () => {
    ...
    if (!remote) {
      ...
      console.log('');
      console.log('  ⚠️  HTTP only — traffic is unencrypted over the network.');
      console.log('      For HTTPS: use Tailscale serve or a reverse proxy (nginx/caddy).');
      console.log('');
      console.log('  Token:        ' + token);
      console.log('');

      // Print primary access URL prominently (first non-loopback LAN IP)
      const primaryIP = (() => {
        const addrs = getNetworkAddresses();
        const lan = addrs.find(a => !a.isTailscale);
        return lan ? lan.address : (addrs[0] ? addrs[0].address : '127.0.0.1');
      })();
      console.log(`  Access URL:   http://${primaryIP}:${port}?token=${token}`);
      console.log('');
