 ...
 * @param {string}  [options.view]      View to capture: 'hourly', 'calendar', or 'list' (default 'list')
...
async function takeSnapshot(options = {}) {
  ...
  const view       = options.view || 'list';  // Default to list view
  ...
  try {
    ...
    // Click the view button if not the default (hourly)
    if (view && view !== 'hourly') {
      const viewBtn = await page.$(`button.view-btn[data-view="${view}"]`);
      if (viewBtn) {
        await viewBtn.click();
        await page.waitForTimeout(300); // Let the view render
      }
    }
